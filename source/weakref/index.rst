...
En python, tous les objets, dans tous les cas sont accèdé par références. Les références sont des pointeurs sur des objets,
ils vont pointer à l'endroit ou se trouve l'objet en mémoire.
...
Le module :py:mod:`weakref` permet de créer et utilisé les weak références sur les objets.
...
.. warning::
    ...
    mais peuvent le faire si l'on les sous classe. Pour plus d'information sur les objets supporter `Weakref support`_.

Garbage collector
"""""""""""""""""

En python l'allocation et la libération de la mémoire se fait automatiquement. Le programmeur n'a pas besoin d'allouer ou de supprimer la mémoire allouer comme en C ou en C++.
La principal stratégie de python pour libérer la mémoire est de se baser sur le système de comptage des références. A chaque fois qu'un référence est créer le compteur de l'objet
s'accrémente et a chaque fois que la référence est supprimer le compteur est décrémenté. C'est seulement quand le compteur est à zéro que la mémoire est liberée.
...
Et que nous executons le code ci-dessous, avec des références fortes sur les objets créer. Il faut que les deux références sur
...
Ce qui va nous permettre d'experimenter les références cyclique (c'est un objet qui a dans ces propriété une référence au même type que lui).
...
Au moment ou l'on veut détruire nos objets, les destructeurs de nos objets a et b ne sont jamais appelé et vont exister tant que l'interpreteur n'est pas quitter.

.. literalinclude:: ./examples/exampleStrongRef2.pycon
   :language: pycon

La solution a ce problème est de stocké des références faible.
